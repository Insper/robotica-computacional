...
Se estiver usando o simulador, certifique-se de que o seu arquivo ***robotica.sh*** está configurado corretamente para se conectar ao robô simulado (Se não sabe do que eu estou falando, volte ao guia robo-simulado).

Se estiver usando o robô físico, configure corretamente o arquivo ***robotica.sh*** (veja o guia robo-real se tiver dúvidas sobre como se conectar ao robô fisico)



## Aruco 3D

Com o aruco 3D utilizando a técnica de marcadores aruco podemos obter os corners que são as coordenadas dos “cantos” do aruco e os IDs dos arucos, também conseguimos calcular o vetor de transalação e o vetor de rotação do aruco em relação ao robô, com essas informações é possível extrair a distância euclidiana e a distância focal do aruco em relação ao robô, essas implementações estão disponíveis para você no código exemplo [aruco_3d.py](aruco_3d.py), para executar:
...
### Tendo isto esclarecido vamos entender a função geraAruco. Cada ponto a baixo irá presentar uma linha do código:


◉ ***Centros:*** Criamos uma lista vazia chamada "centros" para armazenar os centros dos marcadores Aruco detectados.
...
◉ ***grayColor***: É realizada a conversão da imagem colorida de entrada (img) para uma imagem em tons de cinza utilizando a função cv2.cvtColor() da biblioteca OpenCV. 
...
### ❖ A função ***cv2.aruco.getPredefinedDictionary()*** é usada para criar um dicionário contendo os marcadores Aruco pré-definidos, que são um
### conjuntos de marcadores com ID's únicos e padrões específicos. Esses dicionários são usados para detecção e estimativa de posição dos 
...
### Nesta próxima etapa, é feita uma verificação pra identificar se foram encontrados marcadores Aruco na imagem, neste if verificando se a lista ids não está vazia.

```bash
if ids is not None:
```

### Fazemos um ***for*** nos ids detectados na lista ids, onde i é o índice do marcador atual. O condicional dentro do for é utilizado par verifica se o ID do marcador atual é maior do que 99. Se for, é considerado um marcador grande e são utilizados parâmetros diferentes para estimar sua pose.

### A função ***cv2.aruco.estimatePoseSingleMarkers()*** é usada para estimar a pose (orientação e posição) de um marcador Aruco individual a partir dos pontos dos cantos (corners) detectados desse Aruco na imagem. Vamos analisar os parâmetros dessa função:
...
### A linha ***rvec, tvec = ret[0][0,0,:], ret[1][0,0,:]*** está pegando os valores de rotação (rvec) e translação (tvec)
...
***Observação:*** A notação ":" é usada para indicar todas as posições na última dimensão da matriz ou vetor, que no caso é a dimensão 3
...
```bash
                                                                              Exemplo: v = ai+bj , norma = sqrt(a²+b²)
```

### Em resumo, tvec é um vetor de translação estimado para um Aruco na posição 3D da câmera. A norma Euclidiana desse vetor representa a módulo ou distância desse vetor em relação à origem do sistema de coordenadas 3D da câmera. Portanto, np.linalg.norm(tvec) está calculando a distância do aruco em relação à câmera no espaço 3D. A mesma ideia do que foi explicada no if se aplica aqui no else, porém para ids menores que 99
...
### As demais linhas do código são responsáveis por desenha em uma tela a identificação e posição do aruco, mas essas linhas são para critérios visuais e ajudar a facilitar o nosso entendimento.
...
### Estude o código [aruco_3d.py](aruco_3d.py), faça as modificações necessárias para que o robô gire em torno de si mesmo e seja capaz de contar quantos arucos de Id diferentes estão no campo de visão de robô no cenário pista_s2.launch.
