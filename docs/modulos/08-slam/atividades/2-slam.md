...
Essa lógica já foi implementada no arquivo [`amcl.py`](/../../util/amcl.py). Diferente da classe `Odom`, a classe `AMCL` deve ser executada como um nó ROS independente, publicando a pose global estimada no tópico `/amcl_pose`.
